...
阶段 1 的现成素材:`2026-09-16_09-07-17_velocity` 这个 run 有 **201 个 checkpoint(`model_12500` → `model_62250` 间隔 250,外加最后一个 `model_62499`)**,
是一部完整的"学习过程录像"。建议挑 `model_12500` / `25000` / `40000` / `62499` 四个依次回放。
⚠️ 只有最后一个是 `...499`(训练结束时的收尾存档),中间的全是 250 的整数倍。
