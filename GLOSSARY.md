...
| — | Tracking Reward | 跟踪奖励 | "实际值离指令值越近分越高"的奖励,通常是高斯形 exp(−误差²/std²)。**主任务项一般都是它**(本项目:速度跟踪、头部姿态跟踪) | ✓源码 |
| — | Regularization Term | 正则项 | 不定义任务、只约束"怎么做"的奖励项:动作平滑、别晃、别撞自己、别顶关节极限。几乎都是惩罚(≤ 0)。**只靠正则项涨分 = 策略在"什么都不做"** | ✓ |
| — | Joint Limit | 关节极限 | 关节能转到的最大/最小角度。顶着极限用力会损伤真机,所以要惩罚(本项目 `dof_pos_limits`) | ✓ |
